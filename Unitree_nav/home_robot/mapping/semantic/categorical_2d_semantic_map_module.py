...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    https://arxiv.org/pdf/2007.00643.pdfTrue
    ...
    def __init__(
        ...
        self.global_map_lidar_new = torch.zeros(
            1,
            MC.NON_SEM_CHANNELS + self.num_sem_categories,
            self.global_map_size_cm // self.xy_resolution,
            self.global_map_size_cm // self.xy_resolution,           
    ...
    def forward(
        ...
        global_map_lidar:Tensor,
        lobal_map_lidar:Tensor, 
        global_pose_lidar:Tensor, 
        ...
        #从这里开始改######################
        global_pose_lidar[2]= global_pose_lidar[2]*180/3.14159
        #global_pose_lidar=torch.tensor([ 480,  480, 0], device='cuda:0', dtype=torch.float64) ##自定义的机器人在全局中的位置 方位角是角度
        global_pose_lidar[:2] = (global_pose_lidar[ :2] / 100 * self.resolution).int()#修改为地图接受尺度
        self.global_map_lidar_new = self._update_global_map( self.global_map_lidar_new, global_map_lidar,global_pose_lidar  ) 
      
        #从global_map中获取local_map
        local_map_new,local_pose_lidar,lmb_lidar,origins_lidar=mu.recenter_local_map_and_pose_for_env_new( self.global_map_lidar_new,  global_pose_lidar,self.map_size_parameters, )
        seq_local_pose[:, 0] = local_pose_lidar
        seq_global_pose[:, 0] = global_pose_lidar
        seq_lmb[:, 0] = lmb_lidar
        seq_origins[:, 0] = origins_lidar
        
        
        seq_map_features[:, 0] = self._get_map_features(local_map_new, self.global_map_lidar_new)  
        #import pdb;pdb.set_trace()

        return (
            seq_map_features,
            local_map_new,
            self.global_map_lidar_new,
            seq_local_pose,
            seq_global_pose,
            seq_lmb,
            seq_origins,
    ...
    def _update_global_map(
        self, 
        prev_global_map: Tensor,
        global_map_lidar:Tensor,
        global_pose_lidar:Tensor,
        debug: bool = False,
    ) -> Tuple[Tensor, Tensor]:
        global_map_current = torch.zeros(
            1,
            MC.NON_SEM_CHANNELS + self.num_sem_categories,
            self.global_map_size_cm // self.xy_resolution,
            self.global_map_size_cm // self.xy_resolution,
        ...
        global_map_current[0,MC.OBSTACLE_MAP : MC.OBSTACLE_MAP + 1,:,:]=global_map_lidar #障碍物地图0
        maps = torch.cat((prev_global_map.unsqueeze(1), global_map_current.unsqueeze(1)), 1)#torch.Size([1, 2, 10, 480, 480])#将先前地图与更新后的代理视图连接起来
        current_global_map, _ = torch.max(maps, 1)#torch.Size([1, 10, 480, 480])
        curr_loc = global_pose_lidar[ :2]
        curr_loc = (curr_loc * 100.0 / self.xy_resolution).int()
        x, y = curr_loc       
        radius = 10
        explored_disk = torch.from_numpy(skimage.morphology.disk(radius))
        #import pdb;pdb.set_trace()
        current_global_map[ 0, MC.EXPLORED_MAP,    y - radius : y + radius + 1,   x - radius : x + radius + 1,   ][explored_disk == 1] = 1      #靠depth建图(弃用)     探索过   1
        radius = self.been_close_to_radius // self.resolution #100/5=20
        been_close_disk = torch.from_numpy(skimage.morphology.disk(radius))
        
        current_global_map[   0,    MC.BEEN_CLOSE_MAP,        y - radius : y + radius + 1,   x - radius : x + radius + 1,     ][been_close_disk == 1] = 1 # 接近过4
    ...
    def _update_local_map_and_pose(
        ...
        #import pdb;pdb.set_trace()
        tilt = camera_pose[0][1][1]
        agent_height = camera_pose[0][0][2]*100
        ...
        point_cloud_t = du.get_point_cloud_from_z_t( #深度信息生成了点云数据
        ...
        point_cloud_base_coords = du.transform_camera_view_t( #云数据从相机坐标系转换到基本坐标系。
        ...
        point_cloud_map_coords = du.transform_pose_t( #点云数据从基本坐标系转换到地图坐标系。
        ...
        if self.evaluate_instance_tracking:#f
            voxel_channels += self.max_instances + 1

        init_grid = torch.zeros( #创建了一个用于存储体素化数据的张量
            batch_size,
            voxel_channels,
            self.vision_range,
            self.vision_range,
        ...
        feat = torch.ones( #创建了一个用于存储特征数据的张量。
            batch_size,
            voxel_channels,
        ...
        voxels = du.splat_feat_nd(init_grid, feat, XYZ_cm_std).transpose(2, 3)#将特征数据投射到体素网格中。

        agent_height_proj = voxels[ #体素投影1
            ..., self.min_mapped_height : self.max_mapped_height #10，34
        ].sum(4)
        all_height_proj = voxels.sum(4)#体素投影2

        fp_map_pred = agent_height_proj[:, 0:1, :, :]
        fp_exp_pred = all_height_proj[:, 0:1, :, :]
        fp_map_pred = fp_map_pred / self.map_pred_threshold #障碍物地图预测值。
        fp_exp_pred = fp_exp_pred / self.exp_pred_threshold#探索地图预测值
        ...
        agent_view = torch.zeros( #创建了一个用于存储代理视图的张量
            batch_size,
            num_channels,
        ...
        # Update agent view from the fp_map_pred 更新代理视图的障碍物和探索地图
        if self.dilate_obstacles:#t
            for i in range(fp_map_pred.shape[0]): #相对于机器人
                ...
                median_filtered = cv2.medianBlur(env_map, self.dilate_size)#中值滤波
        ...
        agent_view[:, MC.OBSTACLE_MAP : MC.OBSTACLE_MAP + 1, y1:y2, x1:x2] = fp_map_pred #0障碍物 看机器人 将障碍物地图的预测值赋值给代理视图
        agent_view[:, MC.EXPLORED_MAP : MC.EXPLORED_MAP + 1, y1:y2, x1:x2] = fp_exp_pred #1探索过 看所有 将探索地图的预测值赋值给代理视图
        ...
        maps = torch.cat((prev_map.unsqueeze(1), translated.unsqueeze(1)), 1)#torch.Size([1, 2, 10, 480, 480])#将先前地图与更新后的代理视图连接起来

        current_map, _ = torch.max(maps, 1)#torch.Size([1, 10, 480, 480])
        #import pdb;pdb.set_trace()
        if self.record_instance_ids:#f
            # overwrite channels containing instance IDs
            current_map[
                :,
                MC.NON_SEM_CHANNELS #5+5：5+2*5
        ...
        current_map[:, MC.CURRENT_LOCATION, :, :].fill_(0.0)#2当前点
        ...
        for e in range(batch_size):
            ...
            current_map[
                e,
                MC.CURRENT_LOCATION : MC.CURRENT_LOCATION + 2, #2：4
                y - 2 : y + 3,
                x - 2 : x + 3,
            ].fill_(1.0)
        ...
        if debug_maps:
            ...
            explored = current_map[0, MC.EXPLORED_MAP].numpy()#1
            been_close = current_map[0, MC.BEEN_CLOSE_MAP].numpy()#4
            obstacles = current_map[0, MC.OBSTACLE_MAP].numpy()#0
            plt.subplot(331)
            plt.axis("off")
            plt.title("explored")
            plt.imshow(explored)
            ...
            plt.axis("off")
            plt.show()
            plt.savefig("/nanhu-src/activate_inference_habitat/aabbcc.jpg")
    ...
    def _get_map_features(self, local_map: Tensor, global_map: Tensor) -> Tensor:
        ...
        """
        #import pdb;pdb.set_trace()
        map_features_channels = 2 * MC.NON_SEM_CHANNELS + self.num_sem_categories #34 =2*5+24
        ...
        # Local obstacles, explored area, and current and past position   0--4
        map_features[:, 0 : MC.NON_SEM_CHANNELS, :, :] = local_map[:, 0 : MC.NON_SEM_CHANNELS, :, :  ] 
        # Global obstacles, explored area, and current and past position  5--9
        map_features[ :, MC.NON_SEM_CHANNELS : 2 * MC.NON_SEM_CHANNELS, :, :  ] = nn.MaxPool2d(self.global_downscaling)( global_map[:, 0 : MC.NON_SEM_CHANNELS, :, :] )
        # Local semantic categories  10-33  
        map_features[:, 2 * MC.NON_SEM_CHANNELS :, :, :] = local_map[ :, MC.NON_SEM_CHANNELS :, :, : ]
        #0 障碍物地图 1 探索过的区域 2 当前点，3走过的区域 4 走过的区域（圆圈） 0-4局部 5-9全局 10-33局部 
        # plt.imshow(map_features[0,0, :, :].cpu())
