...
def recenter_local_map_and_pose_for_env_new(  
    global_map: Tensor,    
    global_pose: Tensor,   
    map_size_parameters: MapSizeParameters,
):    
    ...
    lmb_lidar = get_local_map_boundaries(global_loc, map_size_parameters)
    origins_lidar = torch.tensor( [ lmb_lidar[2] * p.resolution / 100.0, lmb_lidar[0] * p.resolution / 100.0,    0.0,   ]    )
    local_map = global_map[:,:, lmb_lidar[ 0] : lmb_lidar[ 1], lmb_lidar[ 2] : lmb_lidar[3]]
   # import pdb;pdb.set_trace()
    origins_lidar=origins_lidar.to(global_pose.device)
    local_pose_lidar = global_pose - origins_lidar
    return local_map,local_pose_lidar,lmb_lidar,origins_lidar
