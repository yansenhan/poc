...
class OpenVocabManipAgent(ObjectNavAgent):
    ...
    def __init__(self, config, device_id: int = 0):
        ...
        if config["GROUND_TRUTH_SEMANTICS"]== 0:
            ...
            self.obj_name_to_id, self.rec_name_to_id = read_category_map_file( config["ENVIRONMENT"]["category_map_file"] )
        ...
        if (
            config["AGENT"]["SKILLS"]["NAV_TO_OBJ"]["type"] == "h_rl"
            and not self.skip_skills["nav_to_obj"]
        ):
            from home_robot.agent.ovmm_agent.ppo_agent import PPOAgent
    ...
    def reset_vectorized(self):
        ...
        if self.place_policy is not None:
            ...
            self.place_policy.rotate_num_steps = 24
    ...
    def _init_episode(self, obs: Observations):
        ...
        if self.config["GROUND_TRUTH_SEMANTICS"] == 0:#f
    ...
    def _heuristic_nav(
        ...
        action, planner_info,found_fire ,global_pose_final,local_map_lidar_fire= super().act(obs)
        ...
        if action == DiscreteNavigationAction.STOP:
            ...
            terminate = False
        return action, info, terminate,found_fire,global_pose_final,local_map_lidar_fire
    ...
    def _nav_to_obj(#############################
        ...
        if self.skip_skills["nav_to_obj"]:
            ...
            action, info, terminate,found_fire ,global_pose_final,local_map_lidar_fire= self._heuristic_nav(obs, info)
        elif nav_to_obj_type == "rl":
            #import pdb;pdb.set_trace()
            action, info, terminate = self.nav_to_obj_agent.act(obs, info)
        else:
            raise ValueError(f"Got unexpected value for NAV_TO_OBJ.type: {nav_to_obj_type}")
        new_state = None
        if terminate:
            action = None
            new_state = Skill.GAZE_AT_OBJ
        return action, info, new_state,found_fire,global_pose_final,local_map_lidar_fire
    ...
    def act(
        ...
        while action is None:
            if self.states[0] == Skill.NAV_TO_OBJ:                               
                action, info, new_state,found_fire,global_pose_final,local_map_lidar_fire = self._nav_to_obj(obs, info)               
        ...
        return action, info, obs,found_fire,global_pose_final,local_map_lidar_fire
