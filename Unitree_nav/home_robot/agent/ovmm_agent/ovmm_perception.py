...
class OvmmPerception:
    ...
    def __init__(
        ...
        elif self._detection_module == "vldet":
            from home_robot.perception.detection.VLDet.vldet_perception import (
                VLDetPerception,
            )
            # import pdb; pdb.set_trace()
            self._segmentation = VLDetPerception(
                custom_vocabulary=".",
                sem_gpu_id=gpu_device_id,
                verbose=verbose,
            )
    ...
    def forward(self, obs: Observations, depth_threshold: float = 0.5) -> Observations:
        ...
        # plt.savefig("/nanhu-src/activate_inference_habitat/aabb.jpg")
       # print("dddddddddddddddddddddddddd", obs.semantic.sum())
        #import pdb;pdb.set_trace()
