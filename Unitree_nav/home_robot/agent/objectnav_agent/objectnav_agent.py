...
class ObjectNavAgent(Agent):
    ...
    def prepare_planner_inputs(
            self,
            global_map_lidar:torch.Tensor,
            lobal_map_lidar:torch.Tensor,            
       ...
       # import pdb;pdb.set_trace()
        ...
        return planner_inputs, vis_inputs,self.semantic_map.origins
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        #import pdb;pdb.set_trace()
        planner_inputs, vis_inputs ,origins= self.prepare_planner_inputs(
            torch.tensor(obs.global_map_lidar),
            torch.tensor(obs.local_map_lidar),
            torch.tensor(np.concatenate((np.array(obs.gps),np.array(obs.compass)))),
            obs_preprocessed,
            pose_delta,
        ...
        local_map_lidar=torch.tensor(obs.local_map_lidar)
        #obs_preprocessed[:,5,:,:]=1
        # if ((torch.squeeze(obs_preprocessed[:,5,:,:])==1).sum()>0):#判断rgb中是否找到火源 ######################################
        #     self.found_fire=True
        #     # # import pdb;pdb.set_trace()
        #     # array1=np.where((torch.squeeze(obs_preprocessed[:,5,:,:].cpu())==1))
        #     # len1=np.count_nonzero((torch.squeeze(obs_preprocessed[:,5,:,:].cpu())==1))
        #     # count_left=0
        #     # count_right=0
        #     # count_midden=0
        #     # #import pdb;pdb.set_trace()
        ...
        #     # elif count_midden==0 and count_left>0:
        #     #     action = DiscreteNavigationAction.TURN_LEFT
        #     # elif count_midden==0 and count_right>0:
        #     #     action=DiscreteNavigationAction.TURN_RIGHT
        ...
        #         start_point = (100, 100)  # 箭头的起始点坐标
        #         direction_angle_deg = 45  # 箭头的方向角度，假设是 45 度
        #         arrow_length = 5000  # 箭头的长度

        #         # 计算箭头的终点坐标
        #         direction_angle_rad = np.deg2rad(direction_angle_deg)
        #         end_point = (int(start_point[0] + arrow_length * np.cos(direction_angle_rad)),
        #                     int(start_point[1] + arrow_length * np.sin(direction_angle_rad)))
        #         image=cv2.line(image,start_point,end_point,255,width1)
        #         goal_pt=np.where(image==255 and local_map_lidar==255) #直线和局部地图的交点为火点
        #         if (goal_pt !=NULL) or width1>10:
        #             flag=False
        #             self.found_fire=False
        #         else:
        #             width1+=1                   ##########################################################
                    
        #    closest_goal_pt=goal_pt[0][0]
        
        closest_goal_pt=torch.tensor(closest_goal_pt)
        closest_goal_pt=closest_goal_pt.to(origins.device)
        global_pose_final=closest_goal_pt+origins[0,:2]*20  #局部转全局坐标
        ...
        return action, info,self.found_fire,global_pose_final,local_map_lidar
