...
class DiscretePlanner:
    ...
    def _get_short_term_goal(
        ...
        # goal map
        # 第三阶段，需要找面积最大的
        # if nav_to_recep == 1 and found_goal == True and b3Frontier == False:
        #     goal_map_ = np.where(goal_map == 1, 1, 0)
        #     goal_map_ = goal_map_.astype(np.uint8)
        #     # opencv_rec = cv2.merge([goal_map_])
        #
        #     # 对该连通域进行腐蚀，
        #     kernel = np.ones((3, 3), np.uint8)
        #
        #     goal_map_2 = ndimage.binary_erosion(goal_map_, kernel)
        #     goal_map_3 = ndimage.binary_dilation(goal_map_2, kernel)
        #     # eroded_image = cv2.erode(opencv_rec, kernel, iterations=5)
        #     # opencv_rec = cv2.dilate(eroded_image, kernel, iterations=5)
        #     #
        #     # num_labels, labels = cv2.connectedComponents(opencv_rec, connectivity=8)
        #     # # for i in range(num_labels):
        #     # #     if i > 0:
        #     # #         labels = np.where(labels == i, 0, labels)
        #     # goal_map = np.where(labels > 0, 1, 0)
        #
        #     if np.any(goal_map_3):
        #         goal_map = goal_map_3
