...
from home_robot.agent.ovmm_agent.complete_obs_space import get_complete_obs_space #整个obs空间
...
class PPOAgent(Agent):
    ...
    def act(#############################s
        ...
        #import pdb;pdb.set_trace()
        sample_random_seed()
        if self.skill_start_gps is None:#t
            self.skill_start_gps = observations.gps
        if self.skill_start_compass is None:#t
