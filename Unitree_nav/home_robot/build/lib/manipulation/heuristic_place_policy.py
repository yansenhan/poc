...
class HeuristicPlacePolicy(nn.Module):
    ...
    def __init__(
        ...
        self.rotate_num_steps = 360 // 15  # 计算需要旋转的步数
    ...
    def get_receptacle_placement_point(
        ...
        # arm_reachability_check = True

        NUM_POINTS_TO_SAMPLE = 200  # number of points to sample from receptacle point cloud to find best placement point
        ...
        goal_rec_mask = smooth_mask(goal_rec_mask, self.erosion_kernel, num_iterations=5)[1]
        goal_rec_mask = goal_rec_mask.astype(bool)

        if not goal_rec_mask.any():
            dd = 1
            bFound = False
            return dd, vis_inputs, bFound
        else:
            rgb_vis = obs.rgb
            ...
            print("reachable_point_cloud ", reachable_point_cloud.shape)
            ...
            # )
            random_indices = random.sample(nonzero_tuples, len(nonzero_tuples))
            ...
            print("候选点数量 random_indices ", len(random_indices))
            # 200个区域

            rgb_reach = np.zeros((640, 480), dtype=np.uint8)
            if bshow:
                for i in nonzero_tuples:
                    rgb_reach[i[0], i[1]] = 1
            ...
            print("选出当前的最佳放置点  ", best_voxel_ind)

            # 640*480

            # 判断放置点是否适合放置
            height, width = max_surface_mask.shape[0], max_surface_mask.shape[1]
            mask1 = np.zeros((height, width), dtype=np.uint8)
            radius = 25

            for i in range(height):
                for j in range(width):
                    if (i - best_voxel_ind[0]) ** 2 + (j - best_voxel_ind[1]) ** 2 <= radius ** 2:
                        mask1[i, j] = 1

            #   额外判定放置点一圈内没有干扰物体，有完整的一圈

            if not np.array_equal(max_surface_mask.cpu().numpy(), np.logical_or(mask1, max_surface_mask.cpu().numpy())):
                ...
                if bshow:
                    ...
                    if arm_reachability_check:
                        alpha = 0.7  # 第一个图像的权重
                        # 进行图像融合
                        rgb_vis_tmp = cv2.addWeighted(rgb_vis, alpha, rgb_reach, 1 - alpha, 0)
                        rgb_vis_tmp = cv2.addWeighted(rgb_vis_tmp, ALPHA_VIS, slab_points_mask_vis.cpu().numpy(),
                                                      1 - ALPHA_VIS, 0)
                    else:
                        rgb_vis_tmp = cv2.addWeighted(rgb_vis, ALPHA_VIS, slab_points_mask_vis.cpu().numpy(),
                                                      1 - ALPHA_VIS, 0)
            ...
            # print("保存")
            # print(rgb_vis_tmp)
            # print(rgb_vis_tmp.shape)
            #
            # aa = rgb_vis_tmp
            # binary_image = aa.astype(np.uint8)
            # image = Image.fromarray(binary_image)
            # image.save('/home/zgh/home-robot/datadump/images/yqt/101.bmp')
    ...
    def forward(self, obs: Observations, vis_inputs: Optional[Dict] = None):
        ...
        if self.timestep == 0:
            ...
            dd0, dd1, dd2 = self.get_receptacle_placement_point(obs, vis_inputs)
            print("dd2  ", dd2)
            if dd2 == False:
                vis_inputs = dd1
                print("=====没有合适区域,进行旋转，15度，最多24次")
                if self.rotate_num_steps != 0:
                    action = DiscreteNavigationAction.PLACE_TURN_LIFT
                    self.rotate_num_steps -= 1
                ...
                print("=====存在合适区域，停止旋转，微调")
                self.placement_voxel = dd0
                vis_inputs = dd1
                # 放置点
        ...
        if self.timestep < self.initial_orient_num_turns:
            ...
            action = DiscreteNavigationAction.PLACE_MOVE_FORWARD
        elif self.timestep == self.total_turn_and_forward_steps:
            action = DiscreteNavigationAction.MANIPULATION_MODE


        # elif self.timestep == self.manipulation_mode:
        #     action = DiscreteNavigationAction.NEW_MANIPULATION_MODE
            ...
            delta_arm_ext = delta_arm_ext + 0.25
        ...
        rgb_vis_tmp2 = vis_inputs["semantic_frame"][..., :3]
        aa = rgb_vis_tmp2
        binary_image = aa.astype(np.uint8)
        image = Image.fromarray(binary_image)
        image.save('/home/ubuntu/zyp/home-robot/datadump/images/yqt/102.bmp' + str(self.timestep) + ".bmp")
