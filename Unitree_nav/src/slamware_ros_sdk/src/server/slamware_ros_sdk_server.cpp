...
namespace slamware_ros_sdk {
    ...
    bool SlamwareRosSdkServer::init_(std::string& /*errMsg*/)
    {
        ...
        // init all workers
        {
            ...
            if (0 < params_.odometry_pub_period)
            {
                auto svrWk = boost::make_shared<ServerOdometryWorker>(this, "Odometry", sfConvFloatSecToBoostMs_(params_.odometry_pub_period));
                serverWorkers_.push_back(svrWk);
            }
            ...
            if (0 < params_.robot_basic_state_pub_period)
            {
                ...
                serverWorkers_.push_back(svrWk);
                auto connSvrWk = boost::make_shared<RosConnectWorker>(this, "RosConnectWorker", sfConvFloatSecToBoostMs_(params_.robot_basic_state_pub_period));
                serverWorkers_.push_back(connSvrWk);
            }
            ...
            if(0 < params_.event_period)
            {
                auto svrWk = boost::make_shared<ServerReadEventsWorker>(this, "ReadEvents", sfConvFloatSecToBoostMs_(params_.event_period)); 
                serverWorkers_.push_back(svrWk);
            }
