...
class MapAndScanBuffer:
    def __init__(self):
        ...
        self.count=0
        self.prev_pose=(0,0)
        for x in range(2000):#10m
            for y in range(2000):
                self.boundary_points.append((x, y))
                
        self.local_boundary_points=[]
        ...
        # self.high_cmd = None
        # self.control_listener = rospy.Subscriber('/high_cmd',
        #                                         HighCmd,
        #                                         self.control_fun)
    ...
    def odometry_callback(self, msg):
        ...
        # print("self.cmd_vel == None: ", self.cmd_vel == None)
        # print("self.high_cmd == None: ", self.high_cmd == None)
        # time.sleep(5)
        try:
            cmd_vel = rospy.wait_for_message("/cmd_vel", Twist, timeout=1)
        except:
            cmd_vel = None
            
        try:
            high_cmd = rospy.wait_for_message("/high_cmd", HighCmd, timeout=1)
        except:
            high_cmd = None
        if cmd_vel != None and high_cmd != None: return
        
        print("当前全局位姿:",msg.data,"地图边界宽：",self.global_map.info.width,"地图边界高：",self.global_map.info.height)
        x_map = (msg.data[0] - self.global_map.info.origin.position.x)/self.global_map.info.resolution
        y_map = (msg.data[1] - self.global_map.info.origin.position.y)/self.global_map.info.resolution
        robot_position=(x_map,y_map)
        
        self.local_boundary_points.extend( [(0, y) for y in range(y_map)] + \
                  [(x_map - 1, y) for y in range(y_map)] + \
                  [(x, 0) for x in range(x_map)] + \
                  [(x, y_map - 1) for x in range(x_map)])
       # import pdb;pdb.set_trace()
        
         #初始时，找到最近的边界点
        nearest_point = self.find_nearest_point(robot_position, self.local_boundary_points)     
        flag=True
        while (flag):
            if  self.distance(robot_position, nearest_point) <= 20 :   #20个格子，一个格子0.05m      1m以外的点        
                self.local_boundary_points = self.filter_points(self.local_boundary_points, nearest_point,20)#当前边界点周围1m的边界点屏蔽掉
             #   self.boundary_points  = self.filter_points( self.boundary_points , nearest_point,20)                
                nearest_point = self.find_nearest_point(robot_position, self.local_boundary_points)
            else:
                # if nearest_point not in self.boundary_points:
                #      local_boundary_points = self.filter_points(local_boundary_points, nearest_point,20)
                #      nearest_point = self.find_nearest_point(robot_position, local_boundary_points)
                # else:                    
                    if self.count>20: #点可能在障碍物以外，靠近20次都靠近不了就换一个点
                        self.local_boundary_points = self.filter_points(self.local_boundary_points, nearest_point,20)
                        #self.boundary_points  = self.filter_points( self.boundary_points , nearest_point,20)  
                        nearest_point = self.find_nearest_point(robot_position, self.local_boundary_points)
                        self.count=0
                    else:
        ...
        if self.prev_pose==nearest_point:
            self.count+=1
        
                      
        print("当前导航点:", nearest_point)       
       
      
        global_pose_0=nearest_point[0]*self.global_map.info.resolution+ self.global_map.info.origin.position.x
        global_pose_1=nearest_point[1]*self.global_map.info.resolution+ self.global_map.info.origin.position.y
        print("返回的全局位姿：",global_pose_0, global_pose_1, 0)
        self.send_goals([global_pose_0, global_pose_1, 0])
        # 在这里添加自己的程序，发布每次的目标点，
