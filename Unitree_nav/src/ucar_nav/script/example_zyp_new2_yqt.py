...
class MapAndScanBuffer:
    ...
    def odometry_callback(self, msg):  
        ...
        rl_exploration_dict={}
        rl_exploration_dict["global_map_height"]=self.global_map_info.height
        rl_exploration_dict["global_map_width"]=self.global_map_info.width
        rl_exploration_dict["global_map_array"]=np.array(self.global_map.data).reshape((self.global_map_info.height,  self.global_map_info.width))
        rl_exploration_dict["global_robot_pose_x"]= int((self.robot_x_pose-self.map_info.origin.position.x) /self.map_info.resolution )
        rl_exploration_dict["global_robot_pose_y"]= int((self.robot_y_pose -self.map_info.origin.position.y)/self.map_info.resolution )
        rl_exploration_dict["global_robot_yaw"]=self.robot_yaw
        aa=np.where(rl_exploration_dict["global_map_array"]==99,100,rl_exploration_dict["global_map_array"])
        lidar_map=np.where(aa==100,255,0)        
        kernel=np.ones((25,25))
        kernel=kernel.astype('uint8')
        lidar_map=lidar_map.astype('uint8')
        image=cv2.dilate(lidar_map,kernel)  
        rl_exploration_dict["global_map_obstacle"]=image       #障碍物区域
        rl_exploration_dict["global_map_freespace"]= np.where(self.map_array==0,255,0) #可通行区域
        rl_exploration_dict["x_min_max_for_obs"], rl_exploration_dict["y_min_max_for_obs"]=find_frontiers_min_max_for_obs(self.map_info):  
...
### Exploration Strategy: Start ###
    def find_frontiers_min_max_for_obs(self, map_info):  #所有边界点        
        ...
        x_min_max_for_obs, y_min_max_for_obs = self.get_frontier_max_and_min(frontiers)        
        return  x_min_max_for_obs, y_min_max_for_obs
