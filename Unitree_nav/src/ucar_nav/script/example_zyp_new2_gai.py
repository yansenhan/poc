...
### Exploration Strategy: Start ###
    ...
    def select_goal(self, frontiers, map_data):#边界和地图
        ...
        while True:            
            ...
            if ((self.distance((self.robot_x_pose,self.robot_y_pose), (real_x,real_y))<1)  ):# 机器人距离边界点太近1m以内   到达目标点
                    print("机器人已到达原定边界点，重新选点")
                    ...
                    if ( real_x, real_y) == self.previous_goal:
                        ...
                        if self.distance(self.previous_robot_pose,(self.robot_x_pose,self.robot_y_pose))<0.05:#
                            self.count+=1 
                            print("机器人无法到达该边界点，一共给count次机会，接近次数：",self.count)    
                        else:
                            self.count=0  
                    else:
                            self.count=0  
                    ...
                    if self.count>5:#因为动态障碍物到不了，给5次机会
                        print("重新选点，原定点到不了")
                        self.visited_frontiers_worldpose.append((real_x,real_y)) 
                        for i in range (self.global_map.info.width):
                            for j in range (self.global_map.info.height):
                    ...
                    print("机器人到目标点的距离：",self.distance((self.robot_x_pose,self.robot_y_pose), 
                          "机器人上一步和这一步的差距：",self.distance(self.previous_robot_pose,(self.robot_x_pose,self.robot_y_pose)),(real_x,real_y)),
