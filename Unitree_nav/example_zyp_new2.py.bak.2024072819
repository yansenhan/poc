...
class MapAndScanBuffer:
    ...
    def is_valid_frontier(self, x, y, map_info):
        if not ( 0 <= x < map_info.width and 0 <= y < map_info.height):#超过图像边界
            return False
        if self.near_obstacle(x, y, self.downsampled_map, threshold=3, downsample_factor=2):#周围有障碍物
            return False        
        return True 
    ...
    def select_goal(self, frontiers, map_data):#边界和地图
        ...
        while True:            
            ...
            import pdb;pdb.set_trace()
            if (self.distance((self.robot_x_pose,self.robot_y_pose), (real_x,real_y))<3):#到达目标点  3m以内                           
                    self.visited_frontiers_worldpose.append((real_x,real_y))
                    valid_frontiers.remove(nearest_point)
                    if len(valid_frontiers)>0:
                        nearest_point=self.find_nearest_point((real_robot_x_pose,real_robot_y_pose),valid_frontiers)
